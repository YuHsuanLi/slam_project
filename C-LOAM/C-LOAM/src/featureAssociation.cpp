...
public:
    ...
    void calculateSmoothness()
    {
        ...
        for (int i = 5; i < cloudSize - 5; i++) {
            ...
            float dis_factor = 2.0 / (1.0 + segInfo.segmentedCloudRange[i] / 20.0); // 增加深度因子，根据点深度对粗糙度进行缩放，深度越大，距离比例应该越小；
            if (dis_factor < 0.2) dis_factor = 0.2;         

            cloudCurvature[i] = diffRange*diffRange*dis_factor*dis_factor;
